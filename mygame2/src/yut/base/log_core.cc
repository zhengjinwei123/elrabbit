...
public:
    ...
    using LogLevelVevtor = std::vector<int>;
    using LogFormatterVector = std::vector<LogFormatter>;

    Logger(LogFormatter formatter, int level_filter, int max_log_size);
    ~Logger();

    bool addSink(LogSink *sink, LogFormatter formatter, int level_filter);
    void log(int level, const char *filename, int line, const char *function, const char *format, va_list args);
...
private:
    ...
    LogLevelVevtor sink_level_filters_;
};

////////////////////////////////////////////////////////////////////////////////////
Logger::Logger(LogFormatter formatter, int level_filter, int max_log_size) :
    formatter_(formatter), level_filter_(level_filter), max_log_size_(max_log_size)
{
...
YUT_PRECREATED_SINGLETON_IMPL(LogCore)

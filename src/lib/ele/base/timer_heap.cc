...
namespace ele
{
    namespace timer_heap_impl
    {
        ...
        Timer::Timer(TimerId id, const Timestamp &timestamp, int64_t timeout,
                     const TimerCallback &timer_cb, int call_times) : id_(id),
    ...
    public:
        ...
        TimerId addTimer(const Timestamp &now, int64_t timeout_ms,
                         const TimerCallback &timer_cb, int call_times = -1);
                                                       ...
                                                       const TimerCallback &timer_cb, int call_times)
    {
        ...
        timeout_ms = std::max(static_cast<int64_t>(0), timeout_ms);
        std::unique_ptr<Timer> timer(new Timer(timer_id,
                                               now + timeout_ms,
                                               timeout_ms, timer_cb, call_times));
        minHeapReserve();

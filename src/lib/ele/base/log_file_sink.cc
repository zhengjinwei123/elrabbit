...
namespace ele
{
    ...
    LogFileSink::Impl::Impl(const std::string &file_path) : file_path_(file_path), fp_(nullptr)
    {
    }
    ...
    void LogFileSink::Impl::log(const char *buffer, size_t size)
    {
        if (false == openFile())
        {
            BASE_ERROR("open file %s failed: %s", actual_file_path_.c_str(), ::strerror(errno));
            return;
        }
        ::fwrite(buffer, size, 1, fp_);
    }

    LogFileSink::LogFileSink(const std::string &file_path) : pimpl_(new Impl(file_path))
    {
    }
